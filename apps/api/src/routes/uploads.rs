...
async fn process_upload(
    ...
    let size = i64::try_from(input.bytes.len())
        .map_err(|_| ApiError::Validation("file is too large".into()))?;
    if size <= 0 {
    ...
    let extension = extension_from_name(&original_name)?;
    validate_extension(&preset.allowed_extensions, &extension)?;
    let mime_type = validate_mime(
        &preset.allowed_mime_types,
        input.content_type.as_deref(),
        &input.bytes,
    )?;
    if let Some(session) = &session {
        validate_mime(&session.allowed_mime_types, Some(&mime_type), &input.bytes)?;
    ...
    let hash = hex_digest(&input.bytes);
    ...
    let inserted = file::ActiveModel {
        ...
        metadata_json: Set(json!({ "presetId": preset.id, "sessionId": session.map(|s| s.id) })),
        created_at: Set(now),
        updated_at: Set(now),
    }
    .insert(&state.db)
...
async fn read_multipart(mut multipart: Multipart) -> Result<UploadedPart, ApiError> {
    let mut out = UploadedPart {
        bytes: Vec::new(),
        filename: String::new(),
        content_type: None,
        preset_id: None,
        ...
        .map_err(|e| ApiError::BadRequest(e.to_string()))?
    {
        ...
        if name == "file" {
            out.filename = field.file_name().unwrap_or("upload.bin").to_string();
            ...
            let value = field
                ...
                .map_err(|e| ApiError::BadRequest(e.to_string()))?;
            match name.as_str() {
                "preset_id" => out.preset_id = Some(value),
                "preset" => out.preset = Some(value),
                "project_id" => out.project_id = Some(value),
                _ => {}
            }
        }
    }
